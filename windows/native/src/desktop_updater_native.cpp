...
bool ScheduleInstallAndRelaunchInternal(
...
bool ReadCurrentProductVersionInternal(std::wstring* product_version,
                                       std::string* error) {
  const std::wstring executable_path = CurrentExecutablePath();
...
bool ReadCurrentProductVersion(std::wstring* product_version,
                               std::string* error) {
  return ReadCurrentProductVersionInternal(product_version, error);
}

InstallResult ScheduleInstallAndRelaunch(const InstallRequest& request) {
  std::string error;
  if (!ScheduleInstallAndRelaunchInternal(
          request.staging_path, request.removed_files,
          request.diagnostics_log_path, request.request_elevation_if_needed,
          &error)) {
    return {false, error};
  }
  return {true, ""};
