...
InstallResult ScheduleInstallAndRelaunch(const InstallRequest& request) {
  ...
  script << ")\n"
      ...
      << "    $markerSha256 = [Security.Cryptography.SHA256]::Create()\n"
      << "    try {\n"
      << "      $markerHash = [BitConverter]::ToString($markerSha256.ComputeHash($markerBytes)).Replace('-', '').ToLowerInvariant()\n"
      << "    } finally {\n"
      << "      $markerSha256.Dispose()\n"
      << "    }\n"
      ...
      << "    $stageValidationPhase = 'inventory-entry'\n"
      << "    $entryCount = 0\n"
