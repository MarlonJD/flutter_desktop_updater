...
InstallResult ScheduleInstallAndRelaunch(const InstallRequest& request) {
  ...
  script << ")\n"
      ...
      << "  try {\n"
      << "    $stageValidationPhase = 'preflight'\n"
      ...
      << "    $stageValidationPhase = 'stage-root'\n"
      ...
      << "    $stageValidationPhase = 'marker-hash'\n"
      << "    $markerHash = Get-FileSha256Hex $marker\n"
      << "    $stageValidationPhase = 'marker-compare'\n"
      << "    if ($markerHash -ne $expected_provenance_sha256.ToLowerInvariant()) { throw 'Stage provenance digest changed.' }\n"
      << "    $stageValidationPhase = 'identity'\n"
      ...
      << "    $stageValidationPhase = 'owned-root'\n"
      ...
      << "    $stageValidationPhase = 'inventory-entry-path'\n"
      << "    $entryCount = 0\n"
      ...
      << "      $stageValidationPhase = 'inventory-entry-open'\n"
      << "      $item = Get-Item -LiteralPath $candidate -Force -ErrorAction Stop\n"
      << "      $stageValidationPhase = 'inventory-entry-reparse'\n"
      ...
      << "        $stageValidationPhase = 'inventory-entry-directory'\n"
      << "        if (-not $item.PSIsContainer) { throw 'Stage directory changed.' }\n"
      << "      }\n"
      << "      elseif ($entry.kind -eq 'file') {\n"
      << "        $stageValidationPhase = 'inventory-entry-file-metadata'\n"
      << "        if ($item.PSIsContainer -or $item.Length -ne [int64]$entry.length) { throw 'Stage file changed.' }\n"
      << "        $stageValidationPhase = 'inventory-entry-file-digest'\n"
      << "        $actual = Get-FileSha256Hex $candidate\n"
      << "        if ($actual -ne ([string]$entry.sha256).ToLowerInvariant()) { throw 'Stage file digest changed.' }\n"
      << "      } else {\n"
      << "        $stageValidationPhase = 'inventory-entry-kind'\n"
      << "        throw 'Unsupported Windows stage entry.'\n"
      << "      }\n"
      << "    }\n"
      << "    $stageValidationPhase = 'inventory-count'\n"
      ...
      << "    Write-DiagnosticsEvent ('stage provenance validation phase=' + $stageValidationPhase)\n"
      << "    throw\n"
      << "  }\n"
      << "}\n"
      ...
      << "  $artifactHash = Get-FileSha256Hex $installer\n"
