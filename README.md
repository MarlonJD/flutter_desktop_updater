...
Most Flutter UI integration code can stay the same if your app already uses `DesktopUpdaterController`, `DesktopUpdateWidget`, `DesktopUpdateSliver`, `DesktopUpdateDirectCard`, or `UpdateDialogListener`. The main migration work is in the release artifacts you publish.
...
2. Keep `app-archive.json` as the source of published version/build/platform state. Continue to point each item `url` at the hosted artifact directory for that release.
3. Regenerate every release artifact with the 2.x CLI. Do not reuse macOS artifacts generated by 1.0.0.
4. For Windows and Linux, keep publishing the generated artifact directory with `hashes.json` and all referenced files.
5. For macOS, stop publishing raw `.app` trees or `Contents` directories. Build the app, sign it, notarize it, staple it, then run `dart run desktop_updater:archive macos --app path/to/YourApp.app --channel stable`.
6. Publish the complete macOS artifact directory containing `release-manifest.json`, `payloads/`, and the full fallback `<App>.zip`.
7. Add `"manifest": "release-manifest.json"` to macOS `app-archive.json` items when you want to be explicit. If omitted, the updater defaults to `release-manifest.json`.
8. Make sure the staged macOS app has the same `CFBundleIdentifier` and Apple Developer Team ID as the currently installed app. Updates with a different bundle ID or Team ID are rejected.
9. If your macOS app uses direct bundle replacement, disable the App Sandbox for non-Mac App Store builds or use your own installer/privileged update path.
10. Smoke test from an installed 1.0.0 app to the new hosted release before publishing the `app-archive.json` change broadly.
